...
price = []
item = []
with open(r"C:\Users\abise\OneDrive\Documents\GUI PYTHON\restaurant_details.txt") as f:
    details=f.readlines()
    for i in details:
        w,x=i.split(",")
        y,z=x.split(": ")
...
def Create_order():
    ...
    with open(r"C:\Users\abise\OneDrive\Documents\GUI PYTHON\items.txt","r") as f:
        items=f.readlines()
        item_list=[]
        for i in items:
            i=i[0:(len(i)-2)]
        ...
        def proceed(event=None):
            ...
            if e1.get():
                ...
                if name.isalpha():
                    if e2.get():
                        if e2.get().isdigit():
                            with open(r"C:\Users\abise\OneDrive\Documents\GUI PYTHON\order.txt","a") as file:
...
def find_order():
    ...
    def back():
        ...
        root.deiconify()
    def Total():
        with open(r"C:\Users\abise\OneDrive\Documents\GUI PYTHON\order.txt","r") as f:
            data=f.readlines()
        total=[]
        for i in data:
            j=i.split(",")
